    ...
    <div className="rounded-2xl border border-line glass overflow-hidden flex flex-col" style={{ minHeight: '65vh' }}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 space-y-3">
        {convo.messages.map((m, idx) => {
          const prevName = idx > 0 ? convo.messages[idx - 1].name : null;
          const showNameLabel = m.name !== prevName;
          return (
            <div key={m._id} className="flex flex-col items-start group">
              {showNameLabel && (
                <span className="text-[11px] font-mono text-ink/40 mb-1 ml-1">{m.name}</span>
              )}
...
export default function AdminMessages() {
  ...
  const handleFlush = async () => {
    ...
    try {
      ...
      if (flushed > 0) {
        toast.success(
          `${flushed} New Message${flushed === 1 ? '' : 's'}. ` +
